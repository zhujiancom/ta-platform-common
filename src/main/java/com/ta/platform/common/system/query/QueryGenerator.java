package com.ta.platform.common.system.query;
...
import com.ta.platform.common.tool.SqlInjectionUtil;
...
public class QueryGenerator {
		...
		for (int i = 0; i < origDescriptors.length; i++) {
			...
			try {
				...
				if (null != value && value.toString().startsWith(COMMA) && value.toString().endsWith(COMMA)) {
					...
					final String field = StrUtil.toUnderlineCase(name);
					if(vals.length>1) {
						queryWrapper.and(j -> {
	...
	public static Map<String, SysPermissionDataRuleModel> getRuleMap() {
		...
		List<SysPermissionDataRuleModel> list = WebContext.loadPermissionDataRules();
	...
	public static String installAuthJdbc(Class<?> clazz) {
		...
		for (int i = 0; i < origDescriptors.length; i++) {
			...
			if(ruleMap.containsKey(name)) {
				...
				String filedSql = getSingleSqlByRule(rule, StrUtil.toUnderlineCase(name), value,isString);
	...
	private static String getDbType(){
		...
		try {
			ISysBaseAPI sysBaseAPI = ApplicationContextProvider.getBean(ISysBaseAPI.class);
